...
- **自动审核** — 生成沿用上游的自动／默认审核，不再提供审核强度选择器。
- **瀑布流历史** — 每次生成连同实际参数本地保存，可收藏、可检索
- **灵感库** — 几百个一键套用的高质量提示词

### 商品图

- **原图与产品素材分开选** — 从素材库加入原图不会自动把它选为替换产品。换产品前需单独选择产品素材；原图与产品素材的图片 ID 或图片数据相同时，会在提交生图前拦截。
- **原生遮罩编辑** — 遮罩用于引导换产品或换背景，不保证编辑区域外的像素完全不变。
...
打开右上角账户／应用菜单 → 设置，填一个 OpenAI 或 Gemini 的 API key（baseUrl 留默认）就能生图了。
...
前后端版本不同时，旧版 BFF 未返回的能力保持关闭，已明确返回的能力仍生效；
能力值类型无效时，前端拒绝整份清单。
