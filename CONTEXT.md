...
原画布已离开、会话已切换、发生画布冲突或本机取图失败时，产物留在结果卡中供手动放入，不追写新目标。
手动放入的入口只从「这张产物此刻在不在画布上」反查，不看交付时留下的标记，所以刷新页面、甚至把对象
从画布上删掉之后，入口都还在。结果卡上的图：在画布上的那些由画布渲缩略图（画布仍是这些位图的单源），
不在画布上的回退到服务端按 `taskId` 取原图（视频取封面），所以冲突产出刷新后仍看得见。
...
本机编辑基于较早的项目修订，另一设备已提交更新；两份内容都需保留，不可静默覆盖。它与生成产物准备落画布时的「画布冲突」是两个不同的边界。
_Avoid_: 网络错误、生成失败
