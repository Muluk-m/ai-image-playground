...
以下是已确认、待实现的领域约定，决策见 [ADR 0006](docs/adr/0006-canvas-reading-boundaries.md)。

**读取范围（reading scope）**：
智能体为当前会话可以查找资料的边界，默认包括本会话的整张画布、历史和按既有访问规则可读的素材库；其他会话资料需由用户明确指定后引入。选区表示当前关注对象，不限制画布内的可查找范围。
