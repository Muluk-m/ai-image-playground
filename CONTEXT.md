...
## 画布理解（canvas understanding）

以下是已确认、待实现的领域约定，决策见 [ADR 0006](docs/adr/0006-canvas-reading-boundaries.md)、[ADR 0007](docs/adr/0007-objective-constraints-and-historical-evidence.md) 与 [ADR 0008](docs/adr/0008-project-context-and-execution-windows.md)。

**读取范围（reading scope）**：
智能体为当前创作空间可以查找资料的边界，当前阶段由会话承载，后续由项目承载，包括所属画布、历史和按既有访问规则可读的素材库。选区只表示关注对象；其他创作空间的资料需由用户明确指定后引入。
_Avoid_: 选区、当前上下文、所有用户资料

**画布观察（canvas observation）**：
智能体对特定版本的画布对象及其关系取得的资料与判断。它描述读取时的状态，并不保证对象此后没有变化。
_Avoid_: 实时事实、当前画布、画布副本

**观察过期（stale observation）**：
画布观察所依赖的对象内容、标注或关系已经变化，相关判断需要重新核对的状态。它发生在理解与执行过程中，区别于产物交付时的画布冲突。
_Avoid_: 画布冲突、读取失败、资料缺失

**等待资料（awaiting material）**：
创作所需资料暂时无法取得，智能体保留已有进度、等待资料可用后继续的状态。浏览器关闭造成的本地图片不可读取属于此状态，不代表工作完成。
_Avoid_: 完成、取消、查无对象

**创作目标（creative objective）**：
用户希望对一组指定对象达成的创作结果，可以跨多轮持续推进。同一会话可以包含不同创作目标，目标不等于计费中的对话任务。
_Avoid_: 对话任务、轮、会话

**目标约束（objective constraint）**：
用户针对某个创作目标及其对象提出、在继续修改时沿用的要求。切换创作目标不自动继承旧目标约束；用户明确的长期偏好另行保存。
_Avoid_: 全会话规则、永久偏好

**历史证据（historical evidence）**：
智能体实际读取或生成过、可供后续准确回读的版本资料。删除或替换画布对象不自动删除历史证据，证据清理有独立入口；它与当前发送给模型的内容分开。
_Avoid_: 最新画布、上下文全文、所有编辑快照

## 项目与执行窗口（project & execution window）

以下为后续项目能力的已确认领域方向，尚未引入项目实体或界面；见 [ADR 0008](docs/adr/0008-project-context-and-execution-windows.md)。

**项目（project）**：
拥有一张画布和一套独立项目上下文的创作空间，返回已有项目时恢复它自己的目标与进度。不同项目的工作状态与历史不互相继承，个人创作偏好作为独立的共享默认项提供。
_Avoid_: 执行窗口、创作目标、会话的别名

**项目上下文（project context）**：
归属某个项目的创作目标、约束、历史证据和工作进度等资料的整体。它按需提供给执行窗口，不要求全部同时进入模型输入。
_Avoid_: 全量提示词、当前执行窗口、全用户记忆

**个人创作偏好（personal creative preference）**：
用户明确要求长期沿用、可自动影响不同项目的创作默认项，例如惯用风格；普通任务要求不自动成为个人偏好。偏好更新供之后新建的创作目标使用，进行中的目标延续已有要求，用户针对该目标明确改口时再调整。
_Avoid_: 项目摘要、目标约束、跨项目历史

**执行窗口（execution window）**：
智能体推进创作时使用的一段有容量上限的工作上下文，容量达到上限后通过有界状态交接继续。窗口切换不创建用户可见的独立子会话，也不更换画布；引入项目后窗口归属项目。
_Avoid_: 会话、项目、轮、模型上下文窗口
