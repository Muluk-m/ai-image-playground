...
- 数据源：内置 JSON（bundle 兜底）+ 远程 manifest（GitHub raw，按 `id` 去重远程胜出），5 分钟 localStorage 缓存
- 可通过 `VITE_INSPIRATION_MANIFEST_URL` 覆盖远程源；设为空字符串可禁用远程
...
| `VITE_INSPIRATION_MANIFEST_URL` | 灵感库远程 manifest URL；缺省走硬编码 GitHub raw 公开 mirror repo；空串禁用远程 |
