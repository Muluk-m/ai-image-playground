...
If the gateway's native GPT Image adapter selects an unavailable main model, set `UPSTREAM_IMAGE_RESPONSES_MODEL=gpt-6-astra`. New gateway requests for any `gpt-image-*` model then use `/v1/responses` with that main model and the selected model as the image tool, including references, masks and output controls. This takes precedence over the async-image setting only for these new requests; existing upstream task IDs still resume their original polling path. Leave it empty for native Images behavior. Responses streams have no resumable image-task ID: interruption or timeout is reported as an unknown result rather than automatically generating and charging again.
