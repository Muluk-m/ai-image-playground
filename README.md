...
The script also writes `dist/version.json`, the manifest open tabs poll to notice a new
deployment. Releases are silent by default: running tabs pick the new bundle up on their next
reload. `NOTIFY_UPDATE=true` marks one release as worth interrupting for, and tabs then show a
corner prompt offering a reload.
