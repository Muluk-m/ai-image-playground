...
5. **切换保留身份与数据。** Pages 使用原网页域名，通过 runtime API 配置切换后端并保留各 origin 的 API 映射。保留登录 Cookie、本地画布和原项目对话关联。切换前检查在途任务，迁移前保留数据库备份。runtime 配置只在页面启动时加载，已打开页面可能继续访问旧后端；切回后仍须检查备用新增任务，不能立即停服。
