...
**版本要钉住（2026-09-22 第二次追记）**。`verified` 是活动指针：部署那一刻它指哪就取哪。当天
公开部署比 overlay CI 早 2 分钟取到它，生产落后一档；而 overlay 合并本身又不触发公开部署，
新提交要等公开 main 恰好再动一次才上线。两个仓的流水线互不知道对方，"绑定"只存在于想象里。
现在公开仓有 `private.lock`：生产与 `with-overlay` 只取它；`overlay-bump.yml` 轮询 `verified`，
前移了就把这对组合构建一遍、写进 lock、触发部署。每次生产用的 overlay 版本都在公开历史里。
#728 当时反对钉版本的理由（每次 overlay 前移都要一次公开提交与一整条部署）正是这里要的效果：
overlay 改了就该上线。
