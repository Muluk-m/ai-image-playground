...
**登录态说明（影响本轮覆盖面，务必先读）**：
- **即梦：已实测**（team-lead 用 owner 的 Chrome，含一次 30 积分的真实出片）→ 原始笔记见 **§〇**，结构化分析见 **§十四**。
  ego-browser 侧的 Ego profile**未登录**即梦，所以 §一 里那些靠免登录 API 与公开只读画布得到的事实仍然有效且互为印证。
- **Google Flow：已登录，图片侧与视频侧都做了完整实测**（§四 + §十三，视频侧经 owner 授权用每日免费点数）。
- **可灵、Runway：未登录**，全部为官方文档重建（§三 / §五）。可灵域名在 owner 的 Chrome 扩展里被禁。
- **线索 1 的电商产品：⚪ 仍未能确认名称**（§二 + §十五）。既不在 Ego 的标签页也不在历史里；已实测排除稿定AI、绘蛙、inSai Hilight。
...
| **十一、即梦官方《画布使用手册》** | 🔵 官方全文，补掉即梦一整章的 ⚪ |
| **十二、官方手册带来的结论更新** | 对 §六/§七/§八/§九 的修订，追加 8 条推翻/修正 |
| **十三、Flow 视频侧补测** | 🟢 owner 授权用免费额度实测：**确认卡、思考链、队列、时间线带片段、导出档位** |
| **十四、即梦登录态全程** | 🟢 **实测 · owner Chrome · 含一次 30 积分的真实 Agent 出片** |
| **十五、线索 1 看过原始截图后的更新** | 🟡 logo 是大象；仍 ⚪ 未确认名称 |
| **十六、最终版综合** | **以实测为准，替代 §六/§七/§八 中冲突的部分** |

> **阅读顺序建议：先看 §十六（终版综合）+ §十四（即梦实测）+ §十三（Flow 实测），再回看细节。**
> **§十 的"未能核实"清单写于 §十一 之前，已被 §十一/§十三/§十四 大幅覆盖。**

### 证据来源分工

| 部分 | 谁做的 |
| --- | --- |
| 即梦登录态全程（§十四） | team-lead，用 owner 已登录的 Chrome，花 30 积分 |
| 其余全部（§一~§十三、§十五、§十六） | 本 agent，ego-browser + 免登录 API + 厂商官方文档 |

### 三个数字先放这里

| 产品 | 从零到第一段视频 | 从零到成片 | 生成前确认 / 费用 |
| --- | --- | --- | --- |
| **即梦（画布手动）** | **每镜 6+ 步**，3 镜片子 20+ 步 | 节点右 `+` → 时间线 → 全屏编辑 → 导出 | 价格贴在按钮上（`✦56`），**无确认卡** |
| **即梦（Agent 技能）** | **1 步**（🟢 实测：一句话 → 直接扣 30 积分建 7 节点） | **用户动作 3 次**：输入 / 回「继续」触发拼接 / 进全屏时间线导出 | **零确认、零澄清，钱在第 1 步就花了** |
| **可灵（视频生成页）** | **7 步**（跳过建元素）/ **13 步**（含建元素） | **不存在**——单次 ≤15s，**无时间线** | 有公开秒单价，**无确认卡记载** |
| **可灵（灵动画布）** | **1 步**（官方只承诺到分镜图） | 同上 | 只有顶栏积分余额 |
| **Google Flow** | **3 步**（一句话直接要视频）/ 5 步（先出图再动） | **6 步** | **只对视频确认**（卡片原文「要消耗 7 个点数，开始生成这 1 个视频吗？」+ `批准/一律批准/拒绝`）；**图片不确认，直接生成** |
...
### 4.3 ⚠️ 「生成前先确认」只对**视频**生效，对**图片**直接放行（🟢 两次实测）
...
- ✅ **已核实（owner 授权后用免费额度实测）**：**确认卡只对视频出现。** 见 §十三。
  所以 Flow 的真实策略不是"全局 Ask/Auto"，而是 **「便宜的（图片）直接做，贵的（视频）才问」**。

**对我们的意义**：不要把「生成前确认」当成一个全局开关来抄。**按单价分档才是对的**：
图片这种几毛钱的直接做，视频这种几块钱的才拦一道。**这是本轮对幕芽最直接可用的一条产品裁决。**
...
## 十三、【补测】Google Flow 视频侧全程（🟢 owner 授权，用免费日额度实测）

owner 原话授权：「如果有免费积分随便测，尽量把交互界面都调出来」。本轮**只用免费点数**，
上传图片的使用权条款仍未接受（继续点取消），因此**全程不带参考图**。测试项目**已删除**。

### 13.1 分步走查（接 §4 的步骤 13）

| 步骤 | 用户动作 | 屏幕变化 | 系统替他决定 | 他能改 | 截图 |
| --- | --- | --- | --- | --- | --- |
| 1 | 新建项目 → 在 Agent 输入框写「直接生成一段4秒的视频，不要先出静态图：清晨的咖啡杯放在窗边，镜头缓慢推近，热气升腾。」→ 发送 | 出现**分步状态行**：`Generating Video Directly` → `Refining Model Choice`，每行右侧一个 `>` 展开箭头 | Agent 自行决定模型 | 可点 `停止` | `D30`/`D31-t1` |
| 2 | 约 25 秒后 | **出现确认卡**（见下） | 模型已选定 | `批准` / `一律批准` / `拒绝` | `D31-t2` |
| 3 | 点 `显示思考过程 >` | 展开**完整英文思考链**（见下，含"为什么选这个模型"） | — | — | `D32` |
| 4 | 点 `批准` | 会话里出现一条用户侧气泡 `批准`；Agent 回：「好的，视频生成已经开始了。**由于目前需求量较大，您的视频已进入队列排队中**，预计很快就会开始处理。您可以稍等几分钟再回来查看状态。」；消息里带一个 `▶` 占位缩略图 | — | — | `D33` |
| 5 | 等约 60–90 秒 | **视频完成**：① 会话里的占位图变成可播放缩略图；② **中间资产网格出现视频卡**（带 `▶` 角标 + prompt 作为卡片标题）；③ **左侧导航长出 `视频` 分类**。**注意：Agent 那条"排队中"的消息没有更新**，它一直停在"排队中" | 文件自动用 prompt 命名 | — | `D33-t9` |
| 6 | hover 视频卡 → 点 ⋮ | 菜单比图片多三项：`收藏 / **重复使用提示** / **添加到场景 ▸** / 添加到提示 / **下载 ▸** / 复制 / 重命名 / 分享 / **发布到 YouTube** / 设置项目封面 / 举报输出内容 / 移至回收站` | — | — | `D35` |
| 7 | hover `下载 ▸` | 子菜单：**`270p GIF 动画` / `720p 原始尺寸` / `1080p 已高清重塑` / `4K 已高清重塑`** | **原生只有 720p**，1080p/4K 是**下载时才做的超分**（"已高清重塑"） | 四选一 | `D42` |
| 8 | 点 `添加到场景 ▸` | 子菜单只有一项：**`+ 新建场景`**（已有场景会列在这里） | — | — | `D36` |
| 9 | 点 `+ 新建场景` | 资产网格顶部多出一张**场景卡**；右上角弹 toast：`✓ 已添加到场景` + `切换到场景创建工坊` / `关闭` | 自动建场景并把片段放进去 | — | `D37` |
| 10 | 打开场景卡 | **时间线里有真片段了**：轨道上 00–04 秒是一条**逐帧缩略图带**（filmstrip），右端一个 `⊕` 接下一段；播放器 `00:00:00 / 00:04:00`；右侧栏列出**来源片段卡 + 它的 prompt**；底部输入框 placeholder 变成「**描述如何编辑此视频…**」（空时间线时是「选择要编辑的片段」） | — | 拖动、`⊕` 追加、说一句话改 | `D38` |
| 11 | 顶栏 `↓` | 导出（未点，避免下载） | — | — | — |
...
**和 Runway 对比，这张卡少了两样东西**：

| 字段 | Runway 确认卡 | Flow 确认卡 |
| --- | --- | --- |
| 预估费用 | ✅ estimated credit cost | ✅ **7 个点数** |
| 数量 | ⚪ 未记载 | ✅ **1 个视频** |
| 模型 | ✅ the model | ❌ **不显示**（只在"思考过程"里能读到） |
| prompt | ✅ the prompt | ❌ **不显示**（同上） |
| 「以后都别问了」 | ⚪ 要去设置里改 | ✅ **`一律批准` 直接做在卡上** |

**`一律批准` 做在卡上是个好设计**：用户不用为了"别再烦我"去翻设置面板。

### 13.3 「显示思考过程」里有什么（🟢 `D32`，原文）

> **Generating Video Directly**
> "Okay, I'm focusing on skipping the static image generation step. My goal is a 4-second video, directly creating that zoom on a coffee cup by the window. I'm exploring methods to achieve this without the extra process. Early morning lighting and a slow zoom are key considerations now."
>
> **Refining Model Choice**
> "I've zeroed in on the **Omni 1.1 Flash** model. **It's the only one available given the user's Freemium status** and the 4-second video duration. I'm making sure all constraints, like aspect ratio and resolution, are covered. Now, the main challenge is to keep the original prompt 「清晨的咖啡杯放在窗边，镜头缓慢推近，热气升腾。」**while translating it to a specific English version**."

三条可直接抄的做法：
1. **思考链按阶段起标题**（`Generating Video Directly` / `Refining Model Choice`），折叠默认收起，点开才看细节。
2. **把"为什么选这个模型"说出来**，而且**把套餐限制说出来**（"It's the only one available given the user's Freemium status"）——这比静默降级体验好得多。
3. **中文 prompt 会被翻成英文再送模型**，而且 Agent 自己把"保持原意"当成一个明确约束。

### 13.4 值得注意的三个缺陷（🟢）

1. **进度完全不透明。** Agent 说"已进入队列排队中……您可以稍等几分钟再回来查看状态"，然后**这条消息再也没更新过**。视频做完了，消息还停在"排队中"。用户只能靠自己去资产网格里发现它好了。**没有百分比、没有预计时间、没有状态轮询。**
2. **没有"生成中"的占位节点。** 中间资产网格在生成期间是空的，直到成品出现才"啪"地冒出来。对比即梦（节点原地变成进度态）差很远。
3. **1080p / 4K 是下载时超分，不是原生。** 菜单里写得很老实——`720p 原始尺寸` vs `1080p 已高清重塑`。免费档原生只有 720p。

### 13.5 Flow 的完整数字（更新 §4.4）

| 问题 | Google Flow（更新后） |
| --- | --- |
| 从零到第一段视频 | **3 步**：新建项目 → 写一句话 → **批准**。（先出图再动则是 5 步） |
| 从零到成片（含串联与导出） | **6 步**：上面 3 步 + 资产 ⋮ `添加到场景` → `+ 新建场景` → 场景里 `↓ 下载` |
| 必填决策 | **1 个**：确认卡上的 `批准`。其余全有默认值 |
| 生成前确认 / 费用 | **视频有**（7 点数 / 1 个视频 / 批准·一律批准·拒绝）；**图片没有** |
| 进度呈现 | ❌ **基本没有**——一条不会更新的"排队中"文字 |
| 导出档位 | `270p GIF` / `720p 原始尺寸` / `1080p 已高清重塑` / `4K 已高清重塑`；另有 **`发布到 YouTube`** |
| 改一镜 | 打开片段 → 底部「描述如何编辑此视频…」说一句话（2 步） |

---

## 十四、【补测】即梦登录态全程（🟢 实测 · owner Chrome · 含一次 30 积分的真实 Agent 出片）

由 team-lead 用 owner 已登录的 Chrome 完成，**含一次真实付费**：`/创作分镜` 从一句话跑到时间线成片，花 30 积分。
原始笔记 `notes-jimeng-live.md`。**本节的实测结论优先于 §一 / §十一 / §十二 中基于文档或推断的说法。**

### 14.1 ⚠️ 先更正我自己两条

| 我之前写的 | 实测 | 结论 |
| --- | --- | --- |
| §12.4 #13：「画布里的 Agent 面板『将于后续开放』，即梦对话与画布还没合流」（依据 🔵 官方手册） | **画布右下角有「与 AI 对话」按钮，点了右侧滑出完整会话面板**，能跑技能、能建节点、能建时间线。**已经上线了。** | **官方手册这一句是过期的。实测 > 文档。** 即梦对话与画布**已经合流** |
| §11.11：「导演台 (Beta) 很可能是 3D 场景/机位摆位工具」（推断） | **确认**：`导演台(Beta)` = **「在 3D 空间中设计角色、机位与镜头」**的全屏 3D 预演工具 | **推断成立**，详见 §14.6 |

### 14.2 结构性发现：即梦只有**一个**输入框组件，出现在三处（🟢）

这是整份报告里对幕芽最有参考价值的一条架构事实：
...
- **「创作类型」下拉**（首页）：`Agent 模式 / 图片生成 / 视频生成 / 音乐生成 / 音频生成 / 数字人 / 动作模仿`。
  **图片、视频、音频不是不同页面，而是同一个输入框的一个下拉项。**
- **Agent 模式下参数收起**，底栏只剩 `自动` / `技能` / `@`；**切到「视频生成」底栏立刻展开**为
  `视频生成▾ | 即梦 Seedance 2.0 mini | 全能参考▾ | 16:9·720P·1 | 5s | @ | ✦30(原45) | ↑`，
  placeholder 变成「上传最多 12 个参考素材、输入文字或 @ 参考内容…例如：**@图片1 模仿 @视频1 的动作，音色参考 @音频1**」。
- **参考模式下拉的实测全集**（🟢，比 API 枚举多两项中文名）：
  **`全能参考` / `首尾帧` / `智能多帧` / `智能编辑(Beta)` / `超长视频(Beta)`**。

> **对幕芽的直接含义**：我们现在是「Agent 面板」和「导演台表单」两套 UI。
> 即梦证明了**一套输入框 + 一个「创作类型」下拉 + 一条随类型展开/收起的参数栏**就够了。
> §八 的建议流程应该照这个收敛：**不要为视频再做一个新的输入组件。**

### 14.3 画布模板：即梦官方示范的"正确做法"（🟢）

首页画布 tab 的模板卡 `视频创作` 点开 = 一份**12 节点 6 连线**的模板副本，两个编组：

```
编组A「视频编辑」
  [文本说明节点] + [视频节点：上传参考视频 + 编写文本指令] ──▶ [生成视频结果]

编组B「视频创作」
  [文本说明节点]
  [图片节点：生成场景俯视图 ]──┐
  [图片节点：生成场景        ]──┤
  [图片节点：生成物品三视图  ]──┼──▶ [ 生成视频结果 ]   ← 5 张参考图连进同一个视频节点
  [图片节点：生成物品三视图  ]──┤
  [图片节点：生成人物三视图  ]──┘
```

模板自带的说明原文：**「上传素材 → 生成一致性参考图（人物三视图 / 物品三视图 / 场景俯视图）→ 结合题材产出主体稳定的视频」**。

**这坐实了 §11.4 的判断**：即梦官方推荐的一致性做法就是 **先用图片预设批量造"三视图/场景板"，再把它们全部连进视频节点当参考**。
而且注意 **编组B 里一条 15 秒视频只用了一个节点**——模板里的多镜头**不是多节点**，而是
**一条 prompt 里写分时段脚本**（`0-5s / 5s / 9-10s` + 摄影风格），单次 15s 出完。

> 这是**原型 C（单次多镜型）**在即梦里的官方示范。即梦同时给了 B 和 C 两种做法，模板选了 C。

### 14.4 画布节点的实测细节（🟢，补全 §1.4）

**选中一个已完成的视频节点**，会同时出现三层 UI：

```
        ┌─ 节点上方浮动工具条 ─────────────────────────────────────┐
        │ 局部重拍✦ | 智能超清✦ | 视频编辑✦ | 截取帧▾ | 补帧✦ |    │
        │ 视频修剪 | 提示词反推 | 全屏 | 下载                      │
        └──────────────────────────────────────────────────────────┘
   ●───┤              [ 视频节点 · 内联播放器 ]              ├───●
        ┌─ 底部「视频生成器」──────────────────── [展开视频生成器]┐
        │ [参考缩略图×5：即连入本节点的 5 个上游节点，每个可 Remove]│
        │                                       [+ 添加参考]      │
        │ ┌──────────────────────────────────────────────────────┐│
        │ │ (长 prompt：0-5s … 5s … 9-10s … + 摄影风格)          ││
        │ └──────────────────────────────────────────────────────┘│
        │ 即梦 Seedance 2.5 | 16:9 1080P 1 | 全能参考 | 15s |     │
        │ 引用参考 |                                    [ 生成 ]  │
        └──────────────────────────────────────────────────────────┘
```

**关键：「参考缩略图一排」= 连进来的上游节点。** 连线在 UI 上被具象成生成器顶部的一排可删缩略图——
**连线不是抽象的图论边，它就是"参考列表"的可视化。** 这解释了官方手册那句
「连接完成后，建议在执行任务前**再次确认下游节点实际引用的内容**」。

**右侧 `+` 的下游类型限制**（🟢）：从视频节点右侧 `+` 展开「添加节点」菜单，
**只有 `视频` / `时间线` / `主体` 可选；`文本`/`图片`/`音频`/`导演台` 是置灰的。**
→ **即梦用"上游类型决定下游可选类型"来防止用户连出无意义的图。** 这一条非常值得抄。

画布其余实测：左下 `选择工具 / 小地图 / 显示连线 / 缩放`；右下 `与 AI 对话`；右上 `搜索 / 历史 / 积分`。
...
**= 一个轻量单轨剪辑器。** 素材箱分「已导入资产 / 画布资产」两栏，已用过的标「已添加」。

### 14.6 `导演台 (Beta)` 确认：**3D 预演，不是分镜台**（🟢）

> 官方一句话：**「在 3D 空间中设计角色、机位与镜头」**

全屏 3D 工具，两个模式 `静态预演 | 动态预演`：
- 左侧资产：**人偶预设**（男/女 × 标准/少年/儿童/纤瘦）、**道具**、**图片**、**机位**
- 右侧：对象树 + 属性面板
- 操作：`WASD/QE` 移动、`IJKL` 旋转
- 右上：**截图输出回画布**

**结论：它是一个"构图/机位参考图生成器"——摆好人偶和机位，截一张图回画布当参考。**
和我们那张叫「导演台」的分镜表单**完全不是一回事，只是撞名**。

> **对幕芽的建议不变，而且更清楚了**：我们不用抄这个。
> 但它揭示了一件事——**即梦认为"机位/构图"说不清楚，所以给了一个空间工具**。
> 我们的等价物是更轻的：**在分镜表里给"景别/机位"一个下拉或几个预设词**，不需要 3D。

### 14.7 ⭐ 一次真实的 Agent 出片实测（30 积分，🟢）

输入：`/创作分镜 用画布里的女主和网球拍，做一条 15 秒、3 个镜头的网球拍广告短片，9:16。`
...
| 观察 | 细节 |
| --- | --- |
| **零澄清、零确认** | **没有任何澄清提问，也没有生成前确认卡，直接扣费。** 钱在第 1 步就花了 |
| 它给出的"方案" | 消息里列「将生成以下 3 个视频」，每条 = 完整 prompt（画面 + 光影 + 音效 + **禁止项**）+「已引用的图片: 生成人物三视图」+「**Estimated 10 credits**」 |
| 时长拆分 | 15s 拆成 **3 段 × 5 秒** |
| ⚠️ **没满足用户要求** | 「注：当前模型生成比例**自动适配为 16:9**，生成完成后你可以导出时裁剪为 9:16」——**用户要的 9:16 没做到，而且是事后才说** |
| ⚠️ **拼接要用户再说一句** | 「等全部视频生成好了，**请告诉我继续**，我来帮你把三段视频拼接成完整的 15 秒广告成片」 |
| 底部导航 | 3 个**节点 chip**（网球拍广告…持拍准备 / 挥拍击球 / 得分定格），**点击直接缩放定位到画布上那个节点并选中**，chip 上带状态「生成完成」 |
| 耗时 | 规划 + 提交约 **70 秒**；三段视频 1–2 分钟内全部完成 |
| 画布变化 | 节点数 13 → 16，新节点自动连线到上游参考图 |
| 结算 | 「**本次消耗 30 积分**」；会话自动命名 |

**Agent 建出来的节点参数**（节点输入框可见可改）：
`即梦 Seedance 1.0 Fast | 自动匹配·720P·1 | 首尾帧 | 5s | ✦10`，节点名由 Agent 起（「网球拍广告分镜1：持拍准备」）。

> ⚠️ **一个很硬的质量坑**：**Agent 把「人物三视图」当成了`首尾帧`的首帧**，
> 于是成片第一帧就是那张三视图拼图。
> 「一致性参考图」和「首帧」是两回事，**Agent 自己搞混了**。
> 这正好证明了 §14.2 说的那个参考模式下拉（`全能参考` vs `首尾帧`）**连 Agent 自己都容易选错**。

**拼接**：用户回一句「继续」→ 约 40 秒 → Agent **新建一个时间线节点「15秒网球拍广告成片」**，
把三个视频节点连线进去，消息里给出时间线 chip；**「本次消耗 0 积分」**。
成片导出还要**进时间线全屏编辑器点「导出」**。

### 14.8 即梦两条路径的步数（🟢 实测结论）

**Agent 路径**：
```
1. 输入一句话（含 /技能）      ← 钱在这一步就花了，零确认
2. 等 ~70s 规划 + 1~2min 出片
3. 回一句「继续」              ← 拼接必须用户主动要
4. 点时间线 chip
5. 进全屏编辑器 → 导出
```
**= 用户动作 3 次（输入 / 继续 / 导出），确认点 0 个。**

**手动路径**（画布）：
```
上传或生成参考图节点 → 加视频节点 → 连线 → 写 prompt
→ 选 模型/比例/分辨率/参考模式/时长 → 生成 → 重复 N 镜
→ 节点右 + → 时间线 → 全屏编辑 → 导出
```
**= 每镜 6+ 步，一条 3 镜片子 20+ 步。**
概念负担清单（实测确认）：**节点、连线=参考、参考模式（5 选 1）、主体、时间线节点、导演台(3D)**。

### 14.9 即梦这一轮暴露的四个真实问题（对幕芽全是可避开的坑）

1. **零确认 + 先扣费。** Agent 拿到一句话就直接花 30 积分建 7 个节点并开跑，中途不问一句。
   → **幕芽的分镜表确认卡就是冲这个来的。**
2. **用户的硬约束被静默丢弃。** 用户明确说 9:16，Agent 出了 16:9，**事后才在注释里说"你可以导出时裁剪"**。
   → **幕芽要把"用户显式给的约束"在分镜表上原样显示出来，做不到就在确认前说，不要事后补注。**
3. **拼接不自动。** 三段都好了，Agent 停下来等用户说「继续」。
   → **幕芽的成片条应该在最后一镜完成时自动串好**（拼接不花钱，即梦自己也是 0 积分）。
4. **参考语义 Agent 自己都会选错。** 把三视图当首帧用。
   → **幕芽不暴露"参考模式"这个下拉**（§8.2 的判断得到实证支持）：
   流程规定死"首帧从分镜网格来、参考从 `@` 来"，就没有选错的机会。

---

## 十五、线索 1 电商产品：看过 owner 原始截图后的更新（仍 ⚪ 未能确认名称）

owner 三张截图我已逐张看过（`8.png` 项目页、`9.png` 首页、`10.png` 即梦画布由 team-lead 负责）。

### 15.1 从截图里读到的事实（🟡 owner 截图）

**`8.png`「我的项目」页**：
```
┌──┬──────────────────────────────────────────────────────────────────┐
│🐘│  [ 我的项目 ] [ 人脸资产 ] [ 我的收藏 ]          [视频推荐][运营方板]│
│  │                                                                  │
│创│  [请输入名称        ] [ 全部 ▾ ] [ ⚲ 时间筛选 ]          4 条     │
│意│                                                                  │
│  │  ┌──────────┐  ┌──[视频]───┐  ┌──[图片]───┐                     │
...
- **logo 是一头渐变云朵状的大象**（把 `8.png` 左上角放大 6 倍确认：象耳、象鼻、两只眼睛，白→紫→粉渐变）。截图 `crop-8-logo.png`。
- 左导航：**`创意` / `AI工具` / `品牌` / `我的资产`**；左下依次 **`⊙40 开会员`**、**`领50算力`**、`反馈`、**`API`**、`🔔9`、头像。
- 顶部三个 tab：**`我的项目` / `人脸资产` / `我的收藏`**；筛选栏 `名称搜索 + 全部▾ + 时间筛选`，右侧「4 条」。
- **项目卡左上角角标只有「视频」和「图片」两种**，卡片下面是**项目名 + 修订时间**（不是创建时间）。
- 项目名透露用户画像：`<视频项目 A>`（视频）、`<图片项目 B>`（图片）、
  **`<图片项目 C>`**、`<图片项目 D>`。
  → 项目名显示这是一个面向外贸/消费品卖家的产品。

**`9.png` 首页**：
```
        ┌─────────────────────────────────────┐
        │  [ 🖼 电商图文 ]  [ 🎬 电商视频 ● ]  │   ← 两个大入口（当前选中电商视频）
        └─────────────────────────────────────┘
              ┌──────────────────────────────────────┐
              │ 主图视频  种草视频  品牌TVC  [自由创作●]│   ← 四个子模式
              └──────────────────────────────────────┘
  ┌──────────────────────────────────────────────────────────────────┐
  │ ┌────┐                                                  ☀ 示例   │
  │ │ +  │  上传1–6个参数素材、输入文字或@主体，自由组合图、文、音、  │
  │ │文件│  视频多元素。                                             │
  │ └────┘                                                           │
  │                                                                  │
  │ [⛭ 模型选择] [⚙ 视频参数] [☺ 人脸素材]        [ ✦ 生成  310 算力 ]│
  └──────────────────────────────────────────────────────────────────┘
        口播    种草    品牌    剧情            ← 页面再往下还有一排
```

- **入口按产物分两档**（电商图文 / 电商视频），**再按模板分四档**（主图视频 / 种草视频 / 品牌TVC / 自由创作），当前停在 `自由创作`。
- 输入框底部**只有三个控件**：`模型选择` / `视频参数` / **`人脸素材`**——**「人脸素材」被提到和"模型"同级**，说明数字人口播是它的主场景。
- 生成按钮上**常驻价格 `310 算力`**（和即梦的 `✦56` 是同一个做法）。
- 输入框右上角有 **`示例`**（灯泡图标）——**一键填入示例 prompt**，和可灵的"具名样例 brief"同一招。
- 再往下一排 `口播 / 种草 / 品牌 / 剧情` —— 很可能是作品瀑布流的分类。

### 15.2 排除记录（🟢 实测）

| 候选 | 结论 |
| --- | --- |
| 稿定AI `gaoding.art` | ❌ 左导航 `创作/发现/我的/创建`，货币叫「豆」 |
| 绘蛙 `ihuiwa.com` | ❌ 无左侧竖导航，无双入口（货币确实叫「算力」） |
| **inSai Hilight `hi-light.ai`**（营赛AI，2026-01 腾讯新闻报道的"电商AI视频 DeepSeek 时刻"） | ❌ **logo 是蓝色几何图标不是大象**；站点是英文跨境向，导航 `FEATURES/BLOG/PRICING/ABOUT`，功能是 `AI Avatar / Clothing Try-On / Talking Photo / TikTok Product Video / Hilight Skill / Product Background / Avatar Product Video`。**形态很近但不是它** |

Bing / WebSearch 按「电商图文+电商视频+主图视频+品牌TVC+算力」「人脸资产+算力」「大象 logo + 电商 AI 视频」等六组特征词检索，均未命中。

### 15.3 仍然成立的架构推论

1. **它不自研模型。** 四个子模式与即梦 Agent 内部技能 id 一一对应
   （主图视频=`video-main`、种草视频=`ecom-*`、品牌TVC=`tvc-creation`、电商图文=`image-ecommerce`），
   输入框「1–6 个参考素材、图文音视多元素」就是 Seedance 2.0 `unified_edit`（全能参考）的产品化说法。
2. **它的产品裁决正是 owner 想要的那条**：**入口按产物分（电商图文 / 电商视频），工作面统一（同一块画布/项目，类型只是角标）。**
3. **它把"人脸"提到一级**（`人脸资产` tab + 底栏 `人脸素材`），说明它的主场景是**真人口播带货**，
   这和我们的场景不同——**这一条不要照抄**。

⚪ **仍未能核实**：产品名称与域名；四个子模式各自把用户带到什么中间步骤（**有没有脚本/分镜确认页，这是最想知道的一条**）；
`310 算力` 怎么算；「人脸资产」与「@主体」是否同一个对象；项目内部的画布长什么样。
**只要 owner 给一个 URL 就能全部补上。**

---

## 十六、最终版综合（以实测为准，替代 §六 / §七 / §八 中与之冲突的部分）

### 16.1 跨产品流程对照表（终版）

| 流程阶段 | 即梦 🟢 | 电商产品 B 🟡 | 可灵 🔵 | Google Flow 🟢 | Runway 🔵 |
| --- | --- | --- | --- | --- | --- |
| **起手 / 入口** | 首页 `生成 \| 画布`；**「创作类型」下拉切 Agent/图片/视频/音频/数字人/动作模仿——同一个输入框** | **`电商图文 \| 电商视频` 两个大入口 → 四个模板子模式，进去落同一个项目** | 六张工具卡分流 | 一个 `+ 新建项目`，不分叉 | 左栏 `Agent` → `New chat \| New timeline` |
| **给素材** | 工具栏/双击/拖入/资产库/生成历史。图 30MB、视频 500MB、音频 15MB | 输入框「上传 1–6 个参考素材」 | 视频页传首/尾帧；Omni 传素材 | `+` → 资源选择器 → 上传（**有使用权同意闸门**） | 拖进 prompt，自动编号 `Image 1` |
| **定主体** | **选中画布内容 →「保存为主体」**。图片主体 = 主图 + 3 辅助 + 1 音色(2–5s)；视频主体 = 一个 ≤15s 视频 | `人脸资产` tab + 底栏 `人脸素材` | 元素库（1 主图 + 1–3 补充，可绑音色） | `+ → 创建角色`（描述/上传/从项目添加，6 张原型卡） | 命名上传图 → saved reference，`@[Image 1]` |
| **写脚本 / 分镜** | ① Agent `/创作分镜` 技能（🟢 实测：直接建 N 个节点，不问）② **图片预设 `连续分镜图` → `宫格切分`** ③ **一条 prompt 里写 `0-5s/5s/9-10s` 分时段脚本**（官方模板的做法） | ⚪ | ① prompt 里 `Shot N (Xs)` ② 灵动画布 Agent 出分镜网格 | Agent 在聊天里输出脚本概览（**纯文本，不可编辑**） | Agent plan / `/`Skill / Workflow 节点图 |
| **出首帧 / 关键帧** | 图片节点；`人物/面部/产品三视图` 预设；从视频 `截取帧` | ⚪ | 传首帧；**视频抽帧** | Agent 先出镜头图；或直接跳过出图 | Workflow `Extracting first/middle/last frames` |
| **出视频** | 节点「视频生成器」点生成，**按钮旁常驻 `✦N`** | 生成按钮常驻 **`310 算力`** | 视频页点生成，**按秒计费 6–12 credits/s** | Agent 生成，**有确认卡** | Agent 自动选模型 |
| **生成前确认** | ❌ **没有**。官方口径是"展示预计积分"，实测 Agent **零确认直接扣 30 积分** | ⚪（价格常驻按钮） | ❌ 无记载 | ✅ **视频有**（7 点数 / 1 个视频 / `批准·一律批准·拒绝`）；**图片没有** | ✅ **有但默认关**（model + prompt + est. credits） |
| **进度呈现** | 🟢 节点原地进度 + 会话里的节点 chip 带「生成完成」状态 | ⚪ | ⚪ | ❌ **一条不会更新的"排队中"文字** | `Generations` tab grid |
| **看结果与重试** | 节点历史左右切换 / `设为主视频` / 右键 `应用全部历史` | ⚪ | ⚪ | 会话缩略图 + 资产网格；卡片 `↺` | `Generations` tab 可筛选 |
| **改一镜** | **`局部重拍`（选区重生成）**、`视频编辑`、`修剪`、`补帧`；或改参数再生成（候选不覆盖） | ⚪ | **无专门流程**；整条重生 / Extend / Omni 编辑 / 抽帧重做 | 打开片段 →「**描述如何编辑此视频…**」说一句话（2 步） | 聊天里让 Agent 改（1 步）/ Final Cut 手工 / 资产 `@Agent` |
| **串联 / 拼接** | **时间线是画布上的一个节点** + **全屏单轨剪辑器**（分割/左右裁/删除/吸附/缩放）。Agent 能代劳但**要用户说一句「继续」**，拼接 **0 积分** | ⚪（owner：图片与视频进去是同一块画布） | ❌ **无**。官方立场 "Say goodbye to fragmented assembly" | `资产⋮ → 添加到场景 → + 新建场景`；时间线上 `⊕` 追加 | **`Final Cut` tab，Agent 自动拼** |
| **配音配乐** | 音频节点 `音乐` / `TTS 配音`，**一键加入时间线**；技能 `bgm-search` | 输入框支持「音」元素 | 模型级 Native Audio + 音色绑元素 | 资源选择器有 `语音` 分类 | Agent 自动合成；Final Cut 可 `Split audio` |
| **导出** | **MP4 / XML / 批量素材 / 剪映 / PR / 达芬奇 / FCP**（七选项） | ⚪ | 下载，支持 **.mov** | **`270p GIF` / `720p 原始尺寸` / `1080p 已高清重塑` / `4K 已高清重塑`** + **`发布到 YouTube`** | `Download`，**Agent 产物一律 MP4** |
| **明确没有的** | **延长/续接视频**（官方"计划开发中"）、团队空间 | ⚪ | **时间线** | **节点画布**、**单次多镜** | — |

### 16.2 三种原型（终版判定）

| 产品 | A 对话代办 | B 节点画布 | C 单次多镜 | D 时间线收口 |
| --- | --- | --- | --- | --- |
| **即梦** | ✅ 首页 + **画布右侧会话（实测已上线）** | ✅ 画布节点（深） | ✅ 官方模板就是这么做的（一条 prompt 分时段） | ✅ 时间线节点 + 全屏剪辑器 + 七种导出 |
| **可灵** | ✅ 灵动画布 | ❌ | ✅ Multi-Shot | ❌ |
| **Flow** | ✅ 唯一路径 | ❌ | ❌ | ✅ 场景 |
| **Runway** | ✅ 默认 | ✅ Workflows | ✅ 独立 app（接可灵） | ✅ Final Cut |
| **产品 B** | ✅ 模板化 | ⚪ | ⚪ | ⚪ |

**即梦是唯一四格全打满且全部做深的产品** —— 这就是 owner 说"好复杂"的量化答案。
Flow 只做 A + D，所以最简单；**幕芽应该做 A + D，把 B 的好处（候选不覆盖、逐镜可改）用更轻的方式吸收进 A。**

### 16.3 对幕芽建议流程的最终收敛（在 §八 基础上，被实测改动的四处）

1. **不要为视频做新输入组件。** 即梦证明了一套输入框 + 一个「创作类型」下拉就够（§14.2）。
   幕芽已有 Agent 输入框，**视频只是它的一个创作类型**，参数栏随类型展开/收起。
2. **确认按产物单价分档，不做全局开关。** Flow 实测：图片不问、视频才问（§13.2）。
   幕芽的分镜表卡 `生成 4 张首帧 · 预计 ✦120` 这一步**该问**（要花钱、且分镜是不可逆决策）；
   单张首帧的小改重生成**不要问**。
3. **成片自动串好，不要等用户说"继续"。** 即梦实测里拼接是 0 积分却要用户主动要（§14.7）。
   **幕芽的成片条应在最后一镜完成时自动串好。**
4. **绝不暴露"参考模式"下拉。** 实测里连即梦自己的 Agent 都把「人物三视图」当成了 `首尾帧` 的首帧（§14.7）。
   幕芽流程规定死：**首帧来自分镜网格，参考来自 `@`**，没有选错的机会。

再加一条从即梦学来的**新增建议**：
5. **用"上游类型决定下游可选类型"来约束连线。** 即梦从视频节点的 `+` 只让你接
   `视频 / 时间线 / 主体`，其余置灰（§14.4）。幕芽的成片条与节点之间也应该有同样的类型约束。

### 16.4 最终推荐流程（编号步骤，已吸收全部实测）
...
flowchart LR
  ...
  canvas["同一块画布<br/>左:Agent 右:画布"]
  ref["@ 引用资产库参考图"]
  card["★ 分镜表卡片<br/>#|时长|画面|@主体|景别<br/>[生成分镜网格 · 预计 ✦30]"]
  grid["一张 2x2 分镜网格图<br/>自动切分"]
  frames["N 个镜头节点<br/>各一张首帧"]
  fix["选中 + 说一句话<br/>(遮罩可选) → 多一个候选"]
  clips["[全部出片 · 预计 ✦480]<br/>← 唯一的第二道确认"]
  strip["成片条自动串好<br/>(不等用户说继续)"]
  out["导出 MP4"]

  home -->|两个入口 一块画布| canvas --> ref --> card
  card -->|★ 闸门1：可改表 + 带价| grid --> frames
  frames --> fix -.->|候选不覆盖| frames
  frames --> clips -->|★ 闸门2：带价| strip --> out
  strip -.->|拖动重排/裁切| strip
  card -.->|改表后重铺| frames
```

**用户动作 4 次**（一句话 / 确认分镜表 / 确认出片 / 导出），**确认点 2 个**（都带价格），**必填决策 0 个**。

比即梦少的概念（终版）：`生成/画布` 进门分叉、`导演台`、`参考模式五选一`、`主体独立创建流程`、
`独立时间线页面`、`镜数/总时长下拉`、`连线`（幕芽用分镜表的行序代替连线）。
**七个概念，一个都不需要，因为流程只有一条路，且每一步的输入来源都被流程规定死了。**
