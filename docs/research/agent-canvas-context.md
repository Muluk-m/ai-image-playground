...
用户同时提出“窗口会话”，要求避免上下文无限累积。这确立了有界模型输入的设计方向，窗口如何映射用户可见会话、怎样交接与恢复仍需确认。下文只读投影、工具协议及具体压缩实现继续作为候选，不把设计约定视为已实现能力。
...
建议使用“执行窗口”指代会话内一段有明确容量上限的模型工作上下文，区别于模型参数中的“上下文窗口”。用户可见会话是否保持同一身份，仍需本轮术语确认。创作目标可以跨多轮与多个执行窗口，窗口本身不成为新的计费单位。
