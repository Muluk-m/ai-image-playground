...
第四轮选择 `8A / 9B`：返回已有项目恢复它自身的目标与进度，个人长期创作偏好可跨项目自动生效。个人偏好属于独立共享层，其他项目的工作集、摘要、证据与历史仍不自动进入当前项目。

第五轮选择 `10A / 11A`：只在用户明确表达长期意图时保存个人偏好，普通要求留在当前目标；偏好更新用于之后新建的目标，进行中的目标延续已有要求，当前目标的明确改口优先。下文只读投影、检索与压缩实现仍是候选，不把已确认产品约定视为已实现能力。
...
摘要器不能任意把观察升级为用户要求。目标约束应引用用户原文并绑定创作目标和对象；事实绑定可回读来源；推测单独标记。画布文字、OCR 和检索结果始终保留资料身份，不能经摘要变成系统或用户指令。用户改口保留替代关系，避免旧要求反复复活。长期创作偏好归个人持有、跨项目生效，只有用户明确的长期意图才自动写入，并提供查看、修改和删除入口；临时要求不因项目切换或摘要生成而升格。个人默认值更新用于后续新目标，不自动改变进行中的目标，当前目标的明确要求优先。

#### 个人偏好：结构化存储、按需筛选、文本装配（技术建议）

向量检索与提示词装配处于不同环节：向量用于找出可能相关的记录，被选中的内容最终仍需作为文字或结构化文本进入模型输入。存下 embedding 不会让模型自动记住该偏好，也不会免除读取结果的 token 开销。

首版建议把用户明确的偏好存为结构化记录：`id / owner / key / value / applicability / sourceMessageId / revision / status`。先按归属、目标采用的版本和适用任务类别过滤，处理同键更新与已撤销项，再把少量相关默认项装配为独立偏好块。已有目标约束直接从目标状态加载，不能因语义检索没有命中而丢失。

偏好块标明其用户来源、适用范围与默认地位，不把检索到的原始文本直接提升为系统规则。结构化可识别的当前要求先覆盖对应默认值；自由文本中不明确的冲突应显式识别，不能仅凭相似度分数决定优先级。

目标建立时固定个人偏好资料的版本或快照引用，后续步骤按该版本检索适用项；不需要把全部偏好复制进提示词。这样用户改了全局默认值，返回旧目标仍可保持原要求；目标内明确改口则形成该目标的覆盖要求。

当偏好量和自由文本复杂度增长、基线实验显示规则/关键词漏检时，再考虑带归属与版本过滤的向量或混合检索。它只提供候选召回，不能成为目标约束、访问隔离或偏好优先级的唯一判断。召回结果去重后仍受偏好块和完整执行窗口的双重预算限制，缺少细节时可主动回读。

示例：用户明确要求“以后产品海报默认极简风”，形成适用于产品海报的个人默认项。当前目标明确要求复古风时，窗口使用复古要求；后来把全局默认改为另一风格，进行中的目标仍沿用其原要求。整个过程需要的是可定位、可更新、有范围的偏好记录，首版不必依赖向量数据库。
...
返回已有项目从该项目保存的目标和进度重建有界工作窗口；个人偏好由独立的用户级资料源提供，并同样计入窗口预算。偏好数量增长时也需要去重、作用范围匹配和有界读取，不能把全部用户偏好无条件常驻；窗口化不能只约束项目历史而放任共享记忆增长。
