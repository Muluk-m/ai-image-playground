...
有界模型输入和项目隔离方向已确定；返回已有项目的恢复语义、交接协议与存储方案等继续收敛。下文只读投影、工具协议及具体压缩实现仍是候选，不把设计约定视为已实现能力。
...
摘要器不能任意把观察升级为用户要求。目标约束应引用用户原文并绑定创作目标和对象；事实绑定可回读来源；推测单独标记。画布文字、OCR 和检索结果始终保留资料身份，不能经摘要变成系统或用户指令。用户改口保留替代关系，避免旧要求反复复活。用户明确的长期偏好另行保存，带来源和修改入口；当前没有项目实体，长期偏好的共享范围与管理方式仍需另行确定。
