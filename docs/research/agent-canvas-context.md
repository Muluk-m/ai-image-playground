...
第四轮选择 `8A / 9B`：返回已有项目恢复它自身的目标与进度，个人长期创作偏好可跨项目自动生效。个人偏好属于独立共享层，其他项目的工作集、摘要、证据与历史仍不自动进入当前项目。偏好写入、覆盖关系与更新时机继续收敛；下文只读投影、工具协议及具体压缩实现仍是候选，不把设计约定视为已实现能力。
...
摘要器不能任意把观察升级为用户要求。目标约束应引用用户原文并绑定创作目标和对象；事实绑定可回读来源；推测单独标记。画布文字、OCR 和检索结果始终保留资料身份，不能经摘要变成系统或用户指令。用户改口保留替代关系，避免旧要求反复复活。长期创作偏好已确定归个人持有、跨项目生效，应另行保存并带来源和修改入口；临时目标要求不因项目切换或摘要生成而自动升格。偏好的写入条件、覆盖关系和更新生效时机仍待确认。
