...
本文核对了当时的实现与一手资料。外部资料支持机制选择；具体协议、版本设计与分期是针对本项目的建议，尚未通过真实画布实验验证。

---

> **2026-09-20 抢救归档说明。** 本文原在 PR [#389](https://github.com/Muluk-m/ai-image-playground/pull/389)（draft）里，与三份 ADR 草案和一段 `CONTEXT.md` 改动打包在一起。那三份 ADR 的编号（0006 / 0007 / 0008）在 main 上都已被占用，`CONTEXT.md` 的「项目」定义也与 main 现行术语冲突，因此 **#389 已关闭，只把这份调研正文归档进来**。它是 [#391](https://github.com/Muluk-m/ai-image-playground/issues/391) 的设计依据（#391 正文末尾点名引用），[#408](https://github.com/Muluk-m/ai-image-playground/issues/408) 经 parent 间接继承。
>
> **基线之后 main 已前进 214 个提交，读之前先知道哪些前提已经作废：**
>
> 1. **「当前沿用会话画布」不再成立。** [ADR 0005](../adr/0005-conversation-canvases.md) 已改为「项目画布保留本地可靠性」，项目是画布的稳定归属，#416 已交付本地项目管理。文中凡把「项目」当作未来方向讨论的段落，都要按「项目已是既成实体」重读。
> 2. **`CanvasDoc.editRevision` 这条证据已不存在。** 涉及它的版本失效论证需要重新取证。
> 3. **§5.2 的规模轴（100 / 1,000 / 10,000 对象）当前量不到东西。** 2026-09-20 复核实现后确认：**画布元素数根本不进上下文**，单轮输入对画布对象数 N 是常数级；画布只能经用户附的参考图（`AGENT_TURN_MAX_REFERENCES = 8`，硬上限）间接影响输入。本文主张的「版本化只读投影 + 分层读取工具」一条都没落地，所以那条规模轴要等读取层真的做出来才有意义。**现在该量的是会话消息数 M 与图片的真实 token。**
>
> **仍然成立、可直接拿去用的：**
>
> - §2.5 指出的两道可靠性缺口至今未补：摘要生成后**没有完整请求的最终硬预算检查**；`compaction-transform.ts` 捕获异常后返回原消息，不区分存储故障与预算失败。
> - §2.5 主张的「结果外置先于摘要」仍未实现——`compaction.ts` 里的 `ToolResultOffload` 至今只是类型接缝，注释自述「触发不到，故不实现」。
> - §2.4「确定性的上下文管理器负责预算与必要项保护，不能仅依赖模型『觉得快满了』，也不能用纯 LRU 把早期约束逐出」——这条是设计约束，与规模轴无关。
> - §2.5 的窗口交接概念已在 main 预留成本归属位：`agent_model_calls.purpose` 含 `'handoff'`。
> - §5.3「模型质量与费用目标先测基线后设门槛，不能现在承诺节省某个百分比」——这条直接适用于 #408 的验收写法。
>
> **本文没有给出、也不该被当作已有的东西：** 任何生产阈值（token 预算、节省比例）。原作者明确拒绝虚构这些数字，正确做法是直接查 `agent_model_calls` 的 `usage` 与 `input_image_count` 做实测回归。

---

## 访谈已确认的边界

2026-09-16 第一轮选择 `1A / 2B / 3A`，已记录到 [ADR 0006](../adr/0006-canvas-reading-boundaries.md)：

- 默认读取本会话画布、历史及既有素材库；其他会话由用户明确指定后引入。
- 执行下一项相关操作前核对变化；目标或依赖变化后重读，无关变化不打断，已提交生成保留锁定输入。
- 服务端资料可继续处理，尚未上传的本地图片缺失时保存进度等待，资料可用后继续。

第二轮选择 `4B / 5A / 6A`，记录到 [ADR 0007](../adr/0007-objective-constraints-and-historical-evidence.md)：目标约束随创作目标与对象生效；实际读取/生成的版本资料保留供回读，画布删除不自动清理证据；全画布检查达到费用额度时展示进度并暂停，用户决定是否追加。

第三轮确认 `7A`：执行窗口内部轮换，用户维持同一段连续对话与画布。用户另明确后续引入项目，一个项目对应一张画布和一套独立上下文，切换项目不得携带上一个项目的记忆，记录到 [ADR 0008](../adr/0008-project-context-and-execution-windows.md)。当前会话画布是现状，不再作为未来上下文归属的永久假设。

`8A` 确认返回已有项目恢复它自身的目标与进度。后续 `12A` 将个人创作偏好从首版移出，取代此前 `9B / 10A / 11A` 对首版个人偏好的安排：不做跨项目偏好的写入、管理、检索或自动注入，当前目标的明确要求继续保存。下文只读投影、检索与压缩实现仍是候选，不把已确认产品约定视为已实现能力。

### 当前交付范围

- 渐进读取：区域/对象定位、结构和标注读取、预览与精读。
- 可恢复记忆：当前目标约束、项目内进度和实际参与工作的历史证据。
- 有界执行窗口：输入和交接状态都有上限，自动切换，旧记录按需回读。
- 动态一致性：相关对象变化后重读，已提交生成输入锁定。
- 等待与续做：资料缺失保存进度等待；费用额度用尽暂停并展示覆盖范围。
- 项目兼容：当前沿用会话画布，归属接口兼容后续项目；未来项目切换隔离、返回恢复。

本次不包含项目管理界面及旧数据迁移，也不包含个人创作偏好、为偏好引入的向量库或多智能体递归。窗口容量、压缩水位、读取批量和具体检查点实现由原型与评测验证，不要求用户凭空选择技术参数。
...
窗口化还需改变历史加载链路：`start-turn.ts` 当前取得完整未删除历史后由 `turn.ts` 回放到 Agent；本地安装的 `pi-agent-core 0.85.1` 中，`transformContext` 只替换单次请求的局部消息变量，Agent 状态仍追加原消息。现有摘要记录包含覆盖锚点与折叠次数，没有显式窗口身份或可恢复创作检查点。以上是仓库及本地依赖的静态事实，不代表已经测得线上内存曲线。
...
不能承诺浏览器关闭后还能读取尚未上传的图片。读取结果应明确区分 `ready / pending_upload / unavailable / deleted / stale`；已确认的行为是继续处理已有资料，将依赖缺失内容的工作保存进度并等待，资料可用后继续。阶段性结果可以展示，但不能把资料缺失当作已经完成；完整离线执行需要提前上传整个任务依赖集合。
...
摘要器不能任意把观察升级为用户要求。目标约束应引用用户原文并绑定创作目标和对象；事实绑定可回读来源；推测单独标记。画布文字、OCR 和检索结果始终保留资料身份，不能经摘要变成系统或用户指令。用户改口保留替代关系，避免旧要求反复复活。首版不建立跨项目个人创作偏好，临时要求不因项目切换或摘要生成而升格；有效目标约束直接从目标状态恢复，不能因检索漏命中而消失。
...
“执行窗口”指一段有明确容量上限的模型工作上下文，区别于模型参数中的“上下文窗口”。已确认切换窗口保持同一用户可见对话与画布；后续窗口归属项目，一个项目拥有独立画布和上下文。创作目标可以跨多轮与多个执行窗口，窗口本身不成为新的计费单位。

每次调用都重新检查完整输入上限。当前窗口达到工作容量时，先持久化交接状态，再以该状态和必要资料装配下一窗口；旧窗口的详细消息与工具结果不整体传入下一窗口。保存失败则不提交切换，保留当前可恢复状态。

运行时也只加载当前窗口与有界索引，旧历史通过分页检索恢复；替换每轮全量读取、全量回放历史的现有路径。用户查看完整对话的历史接口与模型输入装配分开，外部记录的保留量不决定单次模型输入或 Agent 内存的大小。

上下文装配、证据索引、检查点和工作集都需要显式归属。当前可由会话承载，接口应允许后续归属项目；切换项目时重新选择该项目的资料来源与窗口，禁止复用上一个项目的摘要或检索缓存内容。项目管理与旧会话迁移另行设计，不因提出项目方向立即改变现有会话画布行为。

返回已有项目从该项目保存的目标和进度重建有界工作窗口。首版窗口输入只装配当前归属下的目标状态与按需读取的资料，不依赖个人偏好块或共享创作记忆；已有素材库仍按原有访问规则按需读取。

交接状态也有独立上限，只包含当前目标与生效约束、下一步、目标对象版本、少量关键证据入口和扫描进度入口。旧窗口的摘要不能逐个叠加；已结束目标进入外部索引，历史证据目录分页读取。对 1,000 个对象的检查，完整覆盖表保存在外部，交接只携带进度概况、游标和必要候选，避免把增长从原文转移到摘要或 ID 清单。

检索回来的内容占用当前窗口的工作集预算：完成使用后可移出并保留回读入口，不能每次检索都永久追加。窗口交接按依赖重新核对观察是否过期；早期约束从带来源的目标状态恢复，避免只对上一段摘要反复再摘要。

切换发生在完整工具交互的安全边界；正在运行的生成操作按已有操作标识衔接，不因切换重新提交。单次结果超过预算时先外置或分页；如果当前目标的必要约束本身装不下，应拆分子步骤或暴露受限状态，不能通过无限套窗口隐藏问题。

窗口容量与费用额度分开：容量触顶通过有界交接自动继续；总费用达到既定额度才按 `6A` 暂停。窗口切换不重置累计费用、扫描覆盖、目标约束或访问范围。具体容量与整理水位通过实际模型、多模态用量和任务质量实验决定。
...
每次观察与操作使用明确版本，执行下一项相关操作前核对其依赖；目标或依赖发生变化时重读并显式更新相关任务状态，无关对象变化不打断。生成输入锁定所读素材版本，已提交任务不因画布变化偷偷换图。结果应用前再校验目标是否仍存在、版本是否符合预期；冲突时保留产物供用户查看或采用既有冲突处理，避免覆盖后来编辑。
