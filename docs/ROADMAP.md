...
1. **已上线的能力未验证，就不许铺新能力。** 2026-09-22 松开一格：拦住 Lane C 的一直是 A2 的计费敞口，而出站硬闸上线、最后两格测试补齐之后，单轮输入每次请求现算上界，敞口已关（见 A2）。仍未验证的是产出质量那一侧——`apps/bff/skills/image/` 下三条电商技能正文自己标着「示例级内容，尚未经过真实产出验证」，所以 C0 可以开工，出图质量的结论仍等 A4 那张失败率表。长画布任务的总成本还没量过（A2 验收未勾），但它不再拦 Lane C：有没有上界由硬闸现算，不靠跑一次长任务来证。
...
- #400 最后两格已收口（2026-09-22）：**「不叠加无限摘要」裁决为不做**（#714 关闭）。`maxIncrementalFolds` 的重做前提是整段历史都还在窗口里，而折得够多的会话必然已经有原文读不回来——触发条件与要保护的场景互斥，线上一次都没生效过（生产两套 `foldCount` 全 0）。裁决依据是产品判断而非等来的数据：这个场景以不断修正图片为主，记得住最近的就够，早先的折淡了可接受。`maxIncrementalFolds` / `foldCount` / `rebuildable` 整套已删，配额 `agent:compaction-max-folds` 进 `RETIRED_QUOTAS`。**另一格是测试**，已随 #764 补齐：`request-budget.test.ts` 钉住「图片按像素进闸」——同样八张图，按预览发放行、按原件发拒发，换成 pi 的每块固定值这道闸就再也分不出两者；`compaction-transform.test.ts` 钉住「摘要超时」——不重问（deadline 是我们自己切的）、复用已存摘要、锚点不前移、只记一次熔断，轮照常推进。至此 #400 七格全部有着落。
...
- ~~这一项不完成，Lane C 不开工。~~ → **2026-09-22 解除**：拦路的是计费敞口，不是这一项的全部。出站硬闸已上线并有测试钉住，单轮输入有可预测的上界；剩下的「跑通一次长任务并记录真实总成本」是量数据，不拦 Lane C（C0 已开工，见下）。
...
### C0 电商模板出图：多视角素材、模板（技能形态）与预置模板（#751）

- [ ] 目标：把「一件产品的几个角度」存成一条素材，把「调好的出图效果」存成一条模板——模板的形态就是技能，钉死生成它的模型与尺寸、写明要几个素材位。之后点一下输入框下方的模板 chip、附上素材就出那个效果，`/assets` 的模板卡上还能选一批素材批量出。旧「模板」改名「提示词」，功能不变。
- 现状：spec #751 已定稿，交互原型已对齐（#753，素材页 / 模板页 / 保存卡片 / 模板 chip / 批量出图五处形态全取 A 版），实现切片 #754–#764 在 `feat/ecom-assets-scenes` 上并行推进——#754 / #757 数据模型、迁移与同步，#755 / #758 保存卡片与 `saveAsset` / `saveLook` 工具，#756 / #759 `create-asset` / `create-look` / `reverse-prompt` 三条技能与素材页 / 模板页入口，#760 / #761 / #762 三处出图（画布 chip、生成模式 chip、批量端点），#763 预置模板随镜像以文件发布，#764 A2 收尾与本条登记。
- 入口方式按 2026-09-19 那条裁决走完了原型这一步（#753），不是跳过原型直接按竞品结论开发。
- 涉及：IndexedDB 升版与 `user_assets` / 新 `user_looks` 同步（视角、参考图与封面的图片本体都走既有 `sync:asset-image-bytes` 配额，不另开一个数）、技能运行时（用户模板就是该用户的技能，名 `look-<id>`，与预置技能同一条加载路径）、智能体保存卡片与收件箱回流、组装函数与批量端点、`/assets` 三页签与 composer 下的 chip 排。
- 与 C1 / C2 / C3 的分工：那三条是把单次编辑做成具名能力，C0 收的是**重复出图**本身——同一条模板换一批素材反复出。两边共用 `editImage` 与现有的草稿 → 确认 → 后台任务 → 计费流程，不新开第二套。
- 门禁：入口要求登录且 `accounts:sync` 打开，否则不出现（与 B1 的存储边界同源：云端只存资产与会话）。
- 验收：一条素材的全部视角随 @ 一起进参考图且胶囊只显示素材名；模板保存后能立刻拿素材试效果并更新同一条而不是越存越多；画布 / 生成模式 / 批量三处组装出来的请求一致；模板钉死的模型下线后标「需重新调试」而不悄悄换模型；预置模板只能用不能改；旧素材与旧提示词原样可用。
