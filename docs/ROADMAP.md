...
**2026-09-19 重订。** 旧的 Phase 0/1/2 时间轴已失真：主线三项里 2.2 标着已交付却没收口、2.3 被自己的进展注释废掉、0.1 的 token 单元漏勾；同时 49 张 open issue 里有 40 张是四条规格下的 ready-for-agent 切片，在旧结构里一条都没出现。本次改为按**准入标准**分道。旧编号保留在「已交付」「已搁置」「已砍 / 降级」三节里，不删除。
...
## 当前工作队列（真实状态，2026-09-19）

49 张 open issue，其中 40 张是四条规格下的实现切片：
...
| 零散 | #380 #312 #222 #162 #78 | 分诊 | #312 是安全项，单独看 |
