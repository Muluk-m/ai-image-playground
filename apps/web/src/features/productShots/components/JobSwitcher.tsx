...
export default function JobSwitcher() {
  ...
  return (
    <div className="mb-4 flex flex-wrap items-center gap-2">
      <span className={LABEL}>{t('job.label')}</span>
                  ...
                  >
                    ...
                    <span className="mt-0.5 block text-[11px] text-gray-500 dark:text-gray-400">
                      {t('job.meta', {
                        ...
                        images: t('job.imageCount', { count: job.images.length }),
                        versions: t('job.versionCount', { count: versionCount(job) }),
                      })}
                    </span>
                  </button>
                  <IconButton
                    label={t('job.delete', { name: job.name })}
                    onClick={() => {
                      closeList()
                      setConfirmDialog({
                        title: t('job.deleteTitle'),
                        message: t('job.deleteMessage', { name: job.name }),
                        tone: 'danger',
