...
function Thumbnail({ element, src }: { element: CanvasEl; src?: string }) {
  ...
  return (
    <span
      aria-hidden="true"
      className={`flex h-8 w-8 shrink-0 items-center justify-center rounded bg-muted text-[11px] ${INK_3}`}
    >
  ...
  return (
    <li>
      <button
        ...
        className={`flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left transition-colors hover:bg-muted ${selected ? ACTIVE_LIST_ROW : ''}`}
