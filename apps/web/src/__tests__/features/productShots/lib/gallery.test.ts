...
function version(id: string): ProductShotVersion {
  return { id, taskId: `task-${id}`, plan: '木质浴室', prompt: 'p', masked: true, createdAt: 1 }
}

function image(imageId: string, patch: Partial<ProductShotImage> = {}): ProductShotImage {
...
describe('naming the exported files', () => {
  it('puts the original order and the version number under the job name', () => {
    expect(shotEntryName('折叠浴缸', 0, 0)).toBe('折叠浴缸/01-v1.png')
    expect(shotEntryName('折叠浴缸', 11, 2)).toBe('折叠浴缸/12-v3.png')
  })

  it('numbers the extra images one version returned', () => {
    expect(shotEntryName('折叠浴缸', 0, 0, 1)).toBe('折叠浴缸/01-v1-2.png')
  })
})
