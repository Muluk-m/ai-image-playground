...
describe('fetchProfileModels', () => {
  ...
  it('rejects when baseUrl is empty', async () => {
    await expect(
      fetchProfileModels({ baseUrl: '   ', apiKey: 'k', kind: 'openai-compat' }),
    ).rejects.toThrow(/API URL/)
  })

  it('rejects when apiKey is empty', async () => {
    await expect(
      fetchProfileModels({ baseUrl: 'https://x.example/v1', apiKey: '', kind: 'openai-compat' }),
  ...
  it('GETs {baseUrl}/models with Bearer auth for openai-compat kind', async () => {
    ...
    const result = await fetchProfileModels({
  ...
  it('throws with API error message on HTTP error', async () => {
    ...
    await expect(
      fetchProfileModels({ baseUrl: 'https://x.example/v1', apiKey: 'bad', kind: 'openai-compat' }),
  ...
  it('throws when response contains no recognizable model list', async () => {
    ...
    await expect(
      fetchProfileModels({ baseUrl: 'https://x.example/v1', apiKey: 'k', kind: 'openai-compat' }),
    ).rejects.toThrow(/未找到/)
  })
})
