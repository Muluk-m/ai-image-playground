...
export function videoChannelFor(modelId: string): PublicChannel | undefined {
  const option = videoModelOptions().find((item) => item.modelId === modelId)
  return option ? getStoredChannel(option.channelId) : undefined
}
