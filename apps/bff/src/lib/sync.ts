...
/** 墓碑与实体记录共用的列；墓碑不带内容，也不带 `lastUsedAt`。 */
function metaValues(
  userId: string,
  change: IncomingChange | SyncTombstone,
  version: number,
  lastUsedAt: number,
) {
  const tombstone = isSyncTombstone(change)
  return {
    user_id: userId,
    id: change.id,
    updated_at: change.updatedAt,
    version,
    deleted_at: tombstone ? change.deletedAt : null,
    last_used_at: tombstone ? null : lastUsedAt,
  }
}

function tombstoneOf(id: string, updatedAt: number, deletedAt: number): SyncTombstone {
...
async function lockSyncVersion(tx: BffTransaction, userId: string): Promise<number> {
  await tx
...
async function mergeTemplates(
  ...
  const table = schema.user_templates
    ...
    async write(change, version, lastUsedAt) {
      const values = {
        ...
        ...(isSyncTombstone(change)
          ? { name: null, prompt: null, asset_ids: null, params: null, created_at: null }
          : {
              name: change.name,
...
async function mergeAssets(
  ...
  const table = schema.user_assets
  const existing = await loadExistingMeta(
    tx,
    table,
    userId,
...
export async function synchronize(
  ...
  return db.transaction(async (tx) => {
    ...
    const allocate: AllocateVersion = () => (version += 1)
