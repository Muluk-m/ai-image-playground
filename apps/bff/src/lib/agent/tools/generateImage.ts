...
export const generateImage = defineAgentTool({
  ...
  guidance:
    '用户要新图时调生图工具，把意图补成完整提示词；细节自行补全，不要用开放式问题反问。方向本身拿不准时用澄清工具给出具体方案让他选。用户写得具体时只整理成简报，不另加创意；只给了一句笼统想法时替他做美术指导：定用途、构图与景别、光线、2-3 个具体配色、材质与画风，让画面有明确取向，并在回复里一句话说明你定了哪些。不添加请求里没暗示的人物、道具、品牌、文案；图中文字写在引号里并要求逐字渲染。张数按用户需求选，未要求多张时只出一张；同一画面的多个版本用 n，不同方向分别调用。不要为同一件事调第二次。',
  parameters,
  onError: 'abort',
