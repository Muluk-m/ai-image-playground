      ...
      await tx.execute(sql`
        ...
        SELECT id, user_id, provider, model, status,
          CASE WHEN status IN ('queued', 'in_progress') AND archive_payload IS NOT NULL THEN 'pending'
            ...
            WHEN status = 'completed' AND EXISTS (SELECT 1 FROM generation_images WHERE generation_id = tasks.id AND role = 'output') THEN 'ready'
            ELSE 'none' END,
        ...
        ON CONFLICT (id) DO UPDATE SET status = excluded.status,
          ...
          revision = excluded.revision, parameters = excluded.parameters, actual_parameters = excluded.actual_parameters, archive_status = excluded.archive_status, error_type = excluded.error_type
      `)
    }
