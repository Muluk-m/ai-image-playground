...
describe('对话轮的预扣', () => {
  it('起轮前按估算的输入与预留的输出预扣一次', async () => {
    ...
    // 预留 2000 输出 token × 5 倍 = 固定 10；剩下的是这条短提示词的输入估算，现在约 2.74。
    ...
    expect(reservations[0]!.unitMultiplier).toBeGreaterThan(12.5)
  ...
  it('运营改了对话单价，下一轮就按新的输出倍数与预留预扣', async () => {
    ...
    // 预留 500 输出 token × 4 倍 = 固定 2；剩下的是同一条提示词的输入估算，现在约 2.74。
    // 原区间 (3, 3.5)，随上面那条同样的 CJK 校正上移；变的只是输入这一半，输出预留没动。
    expect(reservations[0]!.unitMultiplier).toBeGreaterThan(4.5)
