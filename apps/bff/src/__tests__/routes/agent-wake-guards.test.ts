...
describe('唤醒并进排队消息', () => {
  it('folds the wake into the queued message turn instead of waking separately', async () => {
    ...
    const first = await send(conversationId, '画一只橘猫')
    await readFrames(first, 3)
    const [task] = await tasksOf(conversationId)
    // 提交它的那一轮还在说话时任务失败了，用户又排了一句话。
    await fail(task!.id)
    ...
    expect(calls).toHaveLength(3)
    expect(await turnIds(conversationId)).toHaveLength(2)
  })
...
describe('连续自动唤醒上限', () => {
  it(`stops after ${AGENT_MAX_CONSECUTIVE_WAKES} wakes in a row and counts again once the user speaks`, async () => {
    ...
    expect(lastUserInput(calls[10]!)).toContain('一只橘猫 call-5：失败')
  })
})
