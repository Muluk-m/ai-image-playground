...
| `DATABASE_POOL_MAX` | `10` | 本进程最多同时持有的数据库连接数（BFF、worker、Admin 通用），须为正整数，否则拒绝启动；超出的查询排队等待。空闲 30 秒的连接自动关闭。Compose 部署按角色写在 `deploy/compose.app.yaml`（发布脚本 `scripts/rollout-runtime.sh` 同步），`app.env` 里设了也会被覆盖 |
